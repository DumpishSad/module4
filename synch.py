...
from table_one import SpimexTradingResult, engine, session
import time
...
def get_all_bulletin_page_urls():
    ...
    while len(page_data) < LIMIT_FILES:
                ...
                if date_span and date_span.text.strip():
                    ...
                    if match:
                        ...
                        if trade_date.year < START_YEAR:
                            return page_data
                        page_data.append((BASE_URL + link['href'], trade_date))
                        new_data_found = True
                        if len(page_data) >= LIMIT_FILES:
                            break
